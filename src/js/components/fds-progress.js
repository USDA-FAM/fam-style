...
$('body').on('click', '[data-behavior~="fakeSystemTrackAddition"]', function(event) {
  ...
  const $targetValue = $target.closest('.fam-progress').find('.fam-progress__value');
...
$('body').on('click', '[data-behavior~="fakeSystemTrackLoop"]', function(event) {
  ...
  const $self = $(this);
  const $target = $('#' + $self.attr('data-target'));
  const $targetBar = $target.find('.fam-progress__primary');
  const $targetValue = $target.find('.fam-progress__value');
