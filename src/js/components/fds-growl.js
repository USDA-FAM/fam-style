...
var WhiteoutComponent = require('./fam-whiteout');
...
Helper.forEach(growl__closeButtons, function(index, value) {
  ...
  _el.addEventListener('click', function(e){
    // pass associated growl to method
    var _g = Helper.getClosest(e.currentTarget, '.fam-growl');
    growl__dismiss( _g );
  }, false);
});
...
function growl__show(g){
  ...
  if( Helper.hasClass(_growl, 'fam-growl--centered') ){
...
function growl__dismiss(g){
  var _growl = g;
  _growl.className = _growl.className + ' fam-growl--dismissing';
...
function growl__dismissDelay(e){
  ...
  _growl.className = _growl.className.replace(' fam-growl--dismissing','');
